...
pub fn bar_chart_money(ui: &mut egui::Ui, title: &str, rows: &[(String, f64)], max: f64) {
    ...
    for (name, amount) in rows.iter().take(6) {
        ui.horizontal(|ui| {
            ...
            let filled = if max > 0.0 { bar_w * (*amount as f32 / max as f32) } else { 0.0 };
...
pub fn bar_chart(ui: &mut egui::Ui, title: &str, rows: &[(String, usize)], max: usize) {
    ui.label(egui::RichText::new(title).small().weak());
